...
/*
	template<>
	struct variadic_fold<one>
	{
		template
		<
			// stack:

				auto s, auto n, auto op, auto V0,
		...
		static constexpr auto result(CHeap ch, LHeap lh, RHeap rh)
		{
			...
			return variadic_fold<s(next_n)>::template result
			<
				s, next_n, op,
	...
	#define define_permutatic_machine_insert_heap_zero_to_stack_position(S, N, C)					\
		...
		{													\
			...
			<												\
				typename n, auto c, auto d, auto i, auto j,						\
															\
					_ ## N ## _fast_auto_Vs  _ ## C ## _comma  auto... Vs,				\
															\
				auto... Ws, typename... Heaps								\
			>												\
			static constexpr auto result(void(*H0)(auto_pack<Ws...>*), Heaps... Hs)				\
